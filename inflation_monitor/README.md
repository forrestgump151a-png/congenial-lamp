...
- **BEI が 1.8% / 2.0% / 2.2% をまたいだ**とき（上抜け・下抜け）
...
BEI は 2% 近辺で推移しているため、単純な「またいだら通知」では境界での小刻みな
往復で毎日通知が飛びます。そこで各閾値について「上にいる／下にいる」状態を保持し、
**不感帯（BEI: ±0.03pt、アンケート: ±0.5pt）を明確に超えて反対側へ抜けたときだけ**
状態を反転して通知します（シュミットトリガ）。2.01 → 1.99 → 2.01 のような往復では
通知は出ず、1.95 まで明確に下げて初めて「下抜け」を通知します。

## 有効化の手順

1. このブランチをデフォルトブランチにマージする（`schedule` 実行はデフォルトブランチのワークフローのみ動きます）
2. リポジトリの Actions が有効なことを確認
3. **まず診断モードで実データ検証**: Actions タブ → `Japan Inflation Expectations Monitor`
   → `Run workflow` → mode に **`check`** を選んで実行。ログに3指標それぞれの
   取得元ファイル・抽出値・信頼度が出るので、値が公表資料と一致するか目視確認します。
   （`check` は履歴もIssueも一切変更しません）
4. 問題なければ mode `run` で実行、以降は平日朝に自動実行されます
...
# 履歴を変えずに「今ちゃんと取れるか」だけ診断（実データ検証用）
python -m inflation_monitor.monitor check
...
- **開発環境からは3サイトすべてへの外部アクセスがブロックされていたため、実データでの抽出検証は未実施です。** 必ず上記手順3の `check` モードで一度確認してください。ロジック自体は合成データによる19件のユニットテストで検証済みです。
...
- `data/history.json` には初期値として 2026年6月調査分（短観 +2.6%、アンケート 51.3%）を登録済みです。短観は次回 **10/1 公表の9月調査**、アンケートも10月公表分で自動的に新規検知の通知が出ます。
- BEI には初期値を入れていません。初回実行時に出所から取得した値がそのまま基準になり、その時点では閾値通知は出ません（誤った初期値による誤報を避けるため）。
